# hi, i'm hayden!

**IT Infrastructure & Cybersecurity Professional** | High School Senior & IT Intern

I manage(d) 800+ endpoints, servers, and network infrastructure at my local high school while (currently) running a production-grade homelab with ESXi virtualization, Kubernetes clusters, and GitOps workflows. I also conduct security research and bug bounty hunting, following responsible disclosure practices.

Currently preparing for cybersecurity studies at UIUC while building real-world experience in enterprise infrastructure management and application security.

## What I Do

**Infrastructure & DevOps**
- Enterprise infrastructure management (VMware ESXi, Active Directory, network administration)
- Kubernetes orchestration with GitLab CI/CD pipelines and GitOps
- Self-hosted services and homelab architecture
- Traefik reverse proxy, cert-manager, Authentik SSO integration
- Container orchestration and automation

**Security Research**
- Bug bounty hunting with responsible disclosure
- Vulnerability assessment and security testing
- Discovered critical vulnerabilities in production systems (exposed credentials, database access)
- Network security and penetration testing

**Development**
- Full-stack web applications
- Infrastructure as Code (IaC)
- CI/CD pipeline automation
- Self-hosted service deployment and maintenance

## Tech Stack

| Category | Technologies |
|--|--|
| **Languages** | ![TypeScript](https://img.shields.io/badge/TypeScript-blue?labelColor=black&logo=typescript) ![Python](https://img.shields.io/badge/Python-blue?labelColor=black&logo=python) ![JavaScript](https://img.shields.io/badge/JavaScript-yellow?labelColor=black&logo=javascript) ![C#](https://img.shields.io/badge/C%23-lightblue?labelColor=black&logo=csharp) ![Bash](https://img.shields.io/badge/Bash-black?labelColor=black&logo=gnubash) |
| **Container & Orchestration** | ![Kubernetes](https://img.shields.io/badge/Kubernetes-blue?labelColor=black&logo=kubernetes) ![Docker](https://img.shields.io/badge/Docker-blue?labelColor=black&logo=docker) ![Helm](https://img.shields.io/badge/Helm-blue?labelColor=black&logo=helm) |
| **Infrastructure** | ![VMware](https://img.shields.io/badge/VMware%20ESXi-yellow?labelColor=black&logo=vmware) ![Traefik](https://img.shields.io/badge/Traefik-blue?labelColor=black&logo=traefikproxy) ![NGINX](https://img.shields.io/badge/NGINX-green?labelColor=black&logo=nginx) |
| **CI/CD & GitOps** | ![GitLab](https://img.shields.io/badge/GitLab%20CI/CD-orange?labelColor=black&logo=gitlab) ![Git](https://img.shields.io/badge/Git-red?labelColor=black&logo=git) ![ArgoCD](https://img.shields.io/badge/ArgoCD-orange?labelColor=black&logo=argo) |
| **Security & Auth** | ![Authentik](https://img.shields.io/badge/Authentik-red?labelColor=black&logo=authentik) ![cert--manager](https://img.shields.io/badge/cert--manager-purple?labelColor=black&logo=letsencrypt) ![Cloudflare](https://img.shields.io/badge/Cloudflare-orange?labelColor=black&logo=cloudflare) |
| **Monitoring & Observability** | ![Grafana](https://img.shields.io/badge/Grafana-orange?labelColor=black&logo=grafana) ![Prometheus](https://img.shields.io/badge/Prometheus-red?labelColor=black&logo=prometheus) |
| **Databases & Storage** | ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?labelColor=black&logo=postgresql) ![Redis](https://img.shields.io/badge/Redis-red?labelColor=black&logo=redis) ![MariaDB](https://img.shields.io/badge/MariaDB-blue?labelColor=black&logo=mariadb) |
| **Tools & IDEs** | ![JetBrains](https://img.shields.io/badge/JetBrains%20IDEs-black?labelColor=black&logo=jetbrains) ![VSCode](https://img.shields.io/badge/VSCode-teal?labelColor=black&logo=visual-studio-code) ![Arch Linux](https://img.shields.io/badge/Arch%20Linux-blue?labelColor=black&logo=archlinux) |

## Current Systems

**Desktop** 

- [![NixOS](https://img.shields.io/badge/NixOS-25.11-blue?labelColor=black&logo=nixos)](https://archlinux.org)
- AMD Ryzen 9 7950X3D (16c/32t)
- Radeon RX7900XT 20GB

**Laptop**

- [![macOS](https://img.shields.io/badge/macOS-Sequoia-blue?labelColor=black&logo=macos)](https://apple.com)
- Intel i7 6-core
- Radeon Pro 5300M 4GB

**Homelab** 

- [![ESXi](https://img.shields.io/badge/VMware-ESXi%208-yellow?labelColor=black&logo=vmware)](https://www.vmware.com)
- Dual Xeon E5-2670 v2 (20c/40t total)
- Running K8s clusters, GitLab, self-hosted services, Authentik SSO

**Daily Driver** 

2023 Hyundai Elantra Hybrid Limited

## Connect

[![Email](https://img.shields.io/badge/Email-cat@reo.moe-purple?logo=protonmail&style=flat&labelColor=black)](mailto:cat@reo.moe)
[![Email](https://img.shields.io/badge/Email-meow@reoccur.cat-purple?logo=protonmail&style=flat&labelColor=black)](mailto:meow@reoccur.cat)

[![Reddit](https://img.shields.io/badge/Reddit-u/reoccurcat-FF4500?logo=reddit&style=flat&labelColor=black)](https://www.reddit.com/user/reoccurcat/)
![Element](https://img.shields.io/badge/Element-@reoccurcat:catgirl.cloud-darkgreen?logo=element&style=flat&labelColor=black)

---

*Illinois State Scholar | 4.45 GPA | Pursuing Cybersecurity PhD track*

![Metrics](./github-metrics.svg)

![](https://hit.yhype.me/github/profile?user_id=84601340)
